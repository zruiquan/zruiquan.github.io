...
installPath="~/dolphinscheduler" # 这里注意，此安装目录不能与当前配置目录相同
...
拆分服务后，配置文件在 `<bin|alert-server|api-server|master-server|worker-server|tools>/dolphinscheduler_env.sh` 六个文件中可以找到，描述了下列配置：
...
使用上面创建的**部署用户**运行以下命令自动完成分布式部署、运行、并启动，部署后的运行日志将存放在 logs 文件夹内
